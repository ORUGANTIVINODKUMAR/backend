...
# --------------------------------------------------------------------------
    ...
    def build_k1_bookmarks(merger, root, k1_pages, k1_names, extract_text, append_and_bookmark, is_unused_page):
        """
        Groups all K-1 pages by EIN and orders Form 1065 / 1120-S / 1041
        using ACTUAL merged PDF page order (NOT fixed or alphabetical order).
        """

        # Root under Income
        k1_root = merger.add_outline_item("K-1", page_num, parent=root)
        # STEP 0 — Append each real K-1 page once so we know their merged locations
        for ein, pages in k1_pages.items():
            for (p, idx, _) in pages:
                txt = extract_text(p, idx)
                if is_unused_page(txt):
                    continue
                append_and_bookmark((p, idx, "K-1"), k1_root, "", with_bookmark=False)


        # Step 1 — Determine FIRST merged page for each form type ------------
        form_first_page = {}   # { '1065': merged_page_number }

        for ein, pages in k1_pages.items():
            form_type = k1_form_type.get(ein, "1065")
            # ⭐ get earliest REAL merged page number
            mapped = [
                k1_page_map[(p, idx)]
                for (p, idx, _) in pages
                if (p, idx) in k1_page_map
            ]

            # SAFETY: If empty (pages not appended yet), fallback to current merged location
            if not mapped:
                earliest = page_num
            else:
                earliest = min(mapped)
            ...
            # Keep earliest page for this form
            if form_type not in form_first_page or earliest < form_first_page[form_type]:
                form_first_page[form_type] = earliest

        # Step 2 — Sort form types by real page location ---------------------
        sorted_forms = sorted(form_first_page, key=lambda f: form_first_page[f])

        # Step 3 — Create form folders in correct order ----------------------
        form_roots = {}
        for f in sorted_forms:
            anchor_page = form_first_page[f]
            form_roots[f] = merger.add_outline_item(f"Form {f}", anchor_page, parent=k1_root)

        # Step 4 — Add EIN folders under each form ---------------------------
        for ein, pages in sorted(k1_pages.items()):
            company = k1_names.get(ein, f"EIN {ein}")
            form_type = k1_form_type.get(ein, "1065")

            # anchor on first REAL merged page for this EIN
            mapped = [
                k1_page_map[(p, idx)]
                for (p, idx, _) in pages
                if (p, idx) in k1_page_map
            ]
            anchor = min(mapped) if mapped else page_num


            # ⭐ Parent bookmark uses merged page number, not local PDF index
            comp_node = merger.add_outline_item(f"{company} (EIN {ein})", anchor, parent=form_roots[form_type])

            # Add each real K-1 page
            for (p, idx, _) in pages:
                txt = extract_text(p, idx)
