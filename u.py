...
def is_unused_page(text: str) -> bool:
    ...
    return (
       ...
       # or "character when passed through or distributed to its direct or in" in norm
        ...
        #consolidated-1099
        #or "" in norm
        or "widely held fixed investment trusts " in norm
        or "royalty trust and commodity trust holders" in norm
        or "please note that whfits must report income based" in norm
        
        #consolidated-1099
        or "we would like you to note the following items" in norm
        or "qualified dividend distributions include dividends paid by most" in norm
...
def extract_account_number(text: str) -> str:
    ...
    #Account 697296887
    match = re.search(r"Account\s+([\dA-Za-z]+)", text, re.IGNORECASE)
    if match:
...
def has_nonzero_int(text: str) -> bool:
    """
    Detect non-zero interest income.
    Works for OCR text: 'INTEREST INCOME $0.02'
    """
    import re

    t = text.lower()

    # Find interest income lines
    # Example matches:
    # "interest income $0.02"
    # "interest income 0.02"
    # "interest income .02"
    m = re.search(
        r"interest\s+income[^0-9\-]*([-]?\d*\.?\d+)", 
        t
    )
...
def classify_text(text: str) -> Tuple[str, str]:
    ...
    con_unused = [
        ...
        "tax character of the distribution has been allocated based on information"
    ]
...
# --------------------------------------------------------------------------
    ...
    if income:
        ...
        # ALWAYS append the real pages
                    # ✅ New deduplicated logic for Consolidated-1099 issuers
                    seen_forms = set()  # track which forms we’ve already bookmarked for this issuer
                    ...
                    for real_entry in real_entries:
                        ...
                        detected_forms.update(extra_forms)

                        # ✅ only add each form bookmark once per issuer
                        for ft in detected_forms:
                            if ft not in seen_forms:
                                label = ft
                                # optional: simplify descriptions
                                if "Description" in label:
                                    label = label.replace(" Description", "")
                                merger.add_outline_item(label, page_num, parent=forms_node)
                                seen_forms.add(ft)
                                print(f"[Bookmark] Added one-time form bookmark '{label}' under {forms_label}", file=sys.stderr)

                                # append every page regardless (no duplicate form bookmark)
